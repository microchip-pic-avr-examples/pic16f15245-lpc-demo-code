...
• Potentiometer – A 10kΩ potentiometer connected to the RC0 pin is used in labs requiring analog inputs. **For the PIC16F15245 device, RC0 is not an analog input; therefore, the potentiometer output must be routed to RC2.** The steps below can be used to make this new connection:<br />

   1. Locate and remove resistor R33 on the bottom side of the Curiosity board as shown in the figure below:
   ###### R33
   ![R33 Location](images/R33.jpg) <br/>

   2. Solder one end of a jumper wire to the connection labeled 'POT1', and connect the other end to pin RC2 as shown below:
   ###### POT1 to RC2 Jumper connection
   ![RC2 POT1 Connection](images/RC2Jumper.jpg) <br/>
...
The ADC value will be printed on UART TX pin which is connected to pin RB7 through PPS. Connect this pin to the Virtual COM port's TX pin using a jumper wire to use the onboard serial to USB feature.

#### Summary
The PIC16F152 family of devices have an on-board Analog-to-Digital Converter (ADC) with 10 bits of resolution. The converter can be referenced to the device’s VDD or an external voltage reference. This lab references it to VDD. The result from the ADC is represented by a ratio of the voltage to the reference.
